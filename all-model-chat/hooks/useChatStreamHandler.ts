...
export const useChatStreamHandler = ({
    ...
    const getStreamHandlers = useCallback((
        ...
        const streamOnComplete = (usageMetadata?: UsageMetadata, groundingMetadata?: any) => {
            ...
            // Use session state manager for atomic updates
            sessionStateManager.atomicSessionUpdate(
                currentSessionId,
                (session) => {
                    let cumulativeTotal = [...session.messages].reverse().find(m => m.cumulativeTotalTokens !== undefined && m.generationStartTime !== generationStartTimeRef.current)?.cumulativeTotalTokens || 0;
                    ...
                    return {...session, messages: finalMessages, settings: session.settings};
                },
                updateAndPersistSessions,
            ...
            // This is now handled by activeJobsManager in the calling code
        ...
        const streamOnPart = (part: Part) => {
            ...
            // Use session state manager for atomic stream updates
            sessionStateManager.atomicSessionUpdate(
                currentSessionId,
                (session) => {
