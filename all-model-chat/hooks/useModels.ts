...
export const useModels = (appSettings: { apiKey: string | null, apiUrl: string | null, useCustomApiConfig: boolean }) => {
    ...
    const fetchAndSetModels = useCallback(async () => {
        ...
        if (typeof navigator !== 'undefined' && !navigator.onLine) {
            setModelsLoadingError("Offline mode. Using fallback models.");
        } else {
            try {
                ...
                if (error instanceof Error && error.name === 'OfflineError') {
                    setModelsLoadingError(errorMessage);
                } else {
                    setModelsLoadingError(`API model fetch failed: ${errorMessage}. Using fallbacks.`);
                }
            }
        }
    ...
    useEffect(() => {
        ...
        window.addEventListener('online', fetchAndSetModels);
        window.addEventListener('offline', fetchAndSetModels);

        return () => {
            window.removeEventListener('online', fetchAndSetModels);
            window.removeEventListener('offline', fetchAndSetModels);
        };
    }, [fetchAndSetModels]);
