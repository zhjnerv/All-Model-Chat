...
export const Message: React.FC<MessageProps> = React.memo((props) => {
    ...
    const messageContainerClasses = `flex items-start gap-1 sm:gap-2 group ${isGrouped ? 'mt-0.5' : 'mt-1.5 sm:mt-4'} ${message.role === 'user' ? 'justify-end' : 'justify-start'}`;
    const bubbleClasses = `w-fit max-w-[calc(100%-1.5rem)] sm:max-w-3xl lg:max-w-4xl xl:max-w-6xl p-1.5 sm:p-3 rounded-2xl shadow-md flex flex-col min-w-0 ${isModelThinkingOrHasThoughts ? 'sm:min-w-[320px]' : ''}`;
