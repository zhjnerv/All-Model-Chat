...
import { ArrowUp, X, Edit2, Loader2, Mic } from 'lucide-react';
  ...
  return (
    <div className="flex items-center justify-between w-full">
        ...
        <div className="flex flex-shrink-0 items-center gap-1.5 sm:gap-2">
            ...
            {isLoading ? ( 
                <button type="button" onClick={onStopGenerating} className={`${buttonBaseClass} bg-[var(--theme-bg-danger)] hover:bg-[var(--theme-bg-danger-hover)] text-[var(--theme-icon-stop)]`} aria-label={t('stopGenerating_aria')} title={t('stopGenerating_title')}><div className="w-3 h-3 bg-current rounded-sm"></div></button>
            ) : isEditing ? (
                <>
