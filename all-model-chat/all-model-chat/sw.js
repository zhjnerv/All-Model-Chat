...
async function handleStreamRequest(generationId, payload, clientId) {
    ...
    try {
        // If a proxy URL is set, the SW's own fetch handler will intercept this.
...
self.addEventListener('fetch', (event) => {
    ...
    // If a proxy URL is set, intercept all requests to the original Google API domain.
    ...
    // If no proxy, or if it's not a Google API request, proceed with network/cache logic.
    if (API_HOSTS.some(host => new URL(request.url).hostname === host)) {
        // For direct API requests (no proxy), just fetch from network. Do not cache.
            ...
            caches.open(CACHE_NAME).then((cache) => {
                return cache.match(request).then((cachedResponse) => {
                    const fetchPromise = fetch(request).then((networkResponse) => {
                        ...
                        // For navigation requests, fall back to the offline page if available
