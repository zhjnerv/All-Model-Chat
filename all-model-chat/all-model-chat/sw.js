...
async function handleStreamRequest(generationId, payload, clientId) {
    ...
    try {
        // The fetch event listener will handle proxying, so we can use the SDK directly here.
...
self.addEventListener('fetch', (event) => {
    ...
    // Check if the request is for the target API and if a proxy is configured
    if (proxyUrl && API_HOSTS.includes(requestUrl.hostname) && requestUrl.pathname.startsWith('/v1beta')) {
        ...
        const newRequest = new Request(newUrl, {
            ...
            mode: 'cors',
            credentials: 'omit',
            // redirect: 'follow' is the default and correct for this case
        });
        event.respondWith(fetch(newRequest));
        return;
    }

    // If it's an API request but no proxy is set, just fetch it directly.
    if (API_HOSTS.some(host => requestUrl.hostname === host)) {
        event.respondWith(fetch(request));
        return;
    }

    // For all other GET requests, use the cache-first strategy.
            ...
            caches.open(CACHE_NAME).then((cache) => {
                return cache.match(request).then((cachedResponse) => {
                    const fetchPromise = fetch(request).then((networkResponse) => {
                        ...
                        // For navigation requests that fail and are not in cache, serve the offline page (index.html)
